...
handle(Req, State) ->
    ...
    {FilterGenre, _} = cowboy_req:qs_val(<<"genre">>, Req),
    ...
    case FilterGenre of
        ...
        _ ->
            {ok, Res2} = data_merger:get_albums_by_genre_name(FilterGenre)
    end,
        
    io:format("Res2: ~p~n", [Res2]),
    io:format("Page Render: ~p~n", [[AlbumsCount, PagesCount, LimitOnPage, CurrentPage, FilterGenre]]),
